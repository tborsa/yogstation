...
// makes snowmen on the map AGGRESSIVE!!!
/datum/round_event_control/snowmen
	name = "Snowmen Hivemind (Christmas)"
	holidayID = CHRISTMAS
	typepath = /datum/round_event/snowmen
	weight = 25							//forces it to be called, regardless of weight
	max_occurrences = 1
	earliest_start = 5000

/datum/round_event/snowmen/snowmen/start()
	var/sentient_pick
	for(var/mob/living/simple_animal/snowman/frosty in world)
		if(prob(99)) // one of them turns out to be frosty, the non aggressive snowman!
			var/turf/T = get_turf(frosty)
			var/mob/living/simple_animal/hostile/retaliate/snowman/S = new(T)
			S.deathmessage = "slowly falls to the ground, shaking their fist angrily!"
		else
			frosty.name = "happy looking snowman"
			frosty.desc = "No harm here!"
			frosty.deathmessage = "slowly falls to the ground, with a sad frown on their face. A final tear falls down their cheek, gently touching the ground. Maybe next year, old sport?"
			if(!sentient_pick)
				offer_control(frosty)
				sentient_pick = TRUE

/datum/round_event/snowmen/announce()
	priority_announce("Intel has just came in! The station you are occupying was previously controlled by a snowman-race like hivemind. Be careful out there!", "Central Command")
